  ...
  useEffect(() => {
    ...
    const getStars = () => {
      ...
      getDocs(queryCollection).then((res) => {
        ...
        for (let i = 0; i < stars.length; i++) {
          propes.push({ tit: tit[i], stars: stars[i], priceRatio: priceRatio[i]});
        }
        const sorted = propes.sort((a, b) =>
          a.stars < b.stars ? 1 : -1
        );
        setProperties(sorted);
      ...
      <div className="bg-primary  text-center mx-auto ">
        ...
        <Chart
          ...
          series={[
            {
              name: "",
              data: !dataGraph ? (properties.map(item => item.stars) ? properties.map(item => item.stars) : openStar) : (properties.map(item => item.priceRatio)),
            },
          ]}
          ...
          options={{
            ...
            xaxis: {
              tickPlacement: "on",
              categories: properties.map(item => item.tit) ? properties.map(item => item.tit) : title,
